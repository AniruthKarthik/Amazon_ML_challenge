...
class PairFeatureExtractor:
    ...
    def _extract_features_fast(
        ...
        for i in range(total):
            ...
            s1_num = extract_numbers(s1_clean_addr)
            c_num = extract_numbers(cand_clean_addr)
        ...
        out_dict = {
            ...
            "feat_score_exact_name": pairs_df["score_exact_name"].to_numpy(dtype=np.float32) if "score_exact_name" in pairs_df.columns else np.zeros(total, dtype=np.float32),
            "feat_score_exact_core": pairs_df["score_exact_core"].to_numpy(dtype=np.float32) if "score_exact_core" in pairs_df.columns else np.zeros(total, dtype=np.float32),
            "feat_score_tfidf_name": pairs_df["score_tfidf_name"].to_numpy(dtype=np.float32) if "score_tfidf_name" in pairs_df.columns else np.zeros(total, dtype=np.float32),
            "feat_score_tfidf_word": pairs_df["score_tfidf_word"].to_numpy(dtype=np.float32) if "score_tfidf_word" in pairs_df.columns else np.zeros(total, dtype=np.float32),
            "feat_score_tfidf_addr": pairs_df["score_tfidf_addr"].to_numpy(dtype=np.float32) if "score_tfidf_addr" in pairs_df.columns else np.zeros(total, dtype=np.float32),
